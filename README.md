...
1. PostgreSQL'de `bursa_trafik` adında (veya istediğiniz isimde) boş bir veritabanı oluşturun.
2. DBeaver vb. bir araç kullanarak CSV dosyasını `tt_hourlytraffic` tablosu olarak içe aktarın (Import).
3. `database.py` dosyasındaki veritabanı bağlantı bilgilerini kendi sisteminize göre güncelleyin.

### 5. LM Studio Yapılandırması

1. LM Studio'yu açın ve Türkçe yeteneği olan bir model indirin (Örn: `Qwen2.5-7b-instruct`).
2. Sol menüden **"Local Server"** sekmesine gidin.
3. Portu **1234** olarak ayarlayın ve **"Start Server"** butonuna basarak API sunucusunu başlatın.
